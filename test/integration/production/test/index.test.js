...
import { recursiveReadDir } from 'neer/dist/lib/recursive-readdir'
...
describe('Production Usage', () => {
  ...
  it('should output traces', async () => {
    ...
    expect(
      serverTrace.files.some((file) =>
        file.includes('neer/dist/server/send-payload/index.js')
      )
    ).toBe(true)
    ...
    expect(
      serverTrace.files.some((file) =>
        file.includes(
          'neer/dist/shared/lib/page-path/normalize-page-path.js'
        )
      )
    ).toBe(true)
    expect(
      serverTrace.files.some((file) =>
        file.includes('neer/dist/server/render.js')
      )
    ).toBe(true)
    expect(
      serverTrace.files.some((file) =>
        file.includes('neer/dist/server/load-components.js')
      )
    ).toBe(true)
    ...
    if (process.platform !== 'win32') {
      expect(
        serverTrace.files.some((file) =>
          file.includes('neer/dist/compiled/webpack/bundle5.js')
        )
      ).toBe(false)
      expect(
  ...
  it('should contain the Next.js version in window export', async () => {
    ...
    try {
      ...
      expect(version).toBe(require('neer/package.json').version)
    } finally {
      if (browser) {
        await browser.close()
      }
