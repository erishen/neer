import type { AdjustFontFallback, FontLoader } from 'neer/font'
// @ts-ignore
import { calculateSizeAdjustValues } from 'neer/dist/server/font-utils'
// @ts-ignore
import * as Log from 'neer/dist/build/output/log'
// @ts-ignore
...
const downloadGoogleFonts: FontLoader = async ({
  ...
  if (isServer && preload && !callSubsets && !config?.subsets) {
    Log.warn(
      `The ${chalk.bold('neer-font/google')} font ${chalk.bold(
        fontFamily
    ...
    try {
      const { ascent, descent, lineGap, fallbackFont, sizeAdjust } =
        calculateSizeAdjustValues(
          require('neer/dist/server/google-font-metrics.json')[
            fontFamily
          ]
        )
