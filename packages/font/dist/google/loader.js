...
// @ts-ignore
const font_utils_1 = require("neer/dist/server/font-utils");
// @ts-ignore
const Log = __importStar(require("neer/dist/build/output/log"));
// @ts-ignore
const chalk_1 = __importDefault(require("neer/dist/compiled/chalk"));
...
const downloadGoogleFonts = async ({ functionName, data, config, emitFontFile, isDev, isServer, loaderContext, }) => {
    ...
    if (isServer && preload && !callSubsets && !(config === null || config === void 0 ? void 0 : config.subsets)) {
        Log.warn(`The ${chalk_1.default.bold('neer-font/google')} font ${chalk_1.default.bold(fontFamily)} has no selected subsets. Please specify subsets in the function call or in your ${chalk_1.default.bold('next.config.js')}, otherwise no fonts will be preloaded. Read more: https://nextjs.org/docs/messages/google-fonts-missing-subsets`);
    }
    ...
    if (adjustFontFallback) {
        try {
            const { ascent, descent, lineGap, fallbackFont, sizeAdjust } = (0, font_utils_1.calculateSizeAdjustValues)(require('neer/dist/server/google-font-metrics.json')[fontFamily]);
