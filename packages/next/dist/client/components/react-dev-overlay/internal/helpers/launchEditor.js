...
var _chalk = _interop_require_default(require("neer/dist/compiled/chalk"));
var _childProcess = _interop_require_default(require("child_process"));
...
var _shellQuote = _interop_require_default(require("neer/dist/compiled/shell-quote"));
...
function guessEditor() {
    ...
    try {
        if (process.platform === 'darwin') {
            const output = _childProcess.default.execSync('ps x').toString();
            ...
            const output = _childProcess.default.execSync('wmic process where "executablepath is not null" get executablepath').toString();
            ...
            const output = _childProcess.default.execSync('ps x --no-heading -o comm --sort=comm').toString();
...
function launchEditor(fileName, lineNumber, colNumber) {
    ...
    if (process.platform === 'win32') {
        ...
        // launch .exe files.
        p = _childProcess.default.spawn('cmd.exe', [
            '/C',
            editor
        ].concat(args), {
            stdio: 'inherit',
        ...
        if (process.platform === 'darwin') {
            p = _childProcess.default.spawn('osascript', [
                '-e',
        ...
        p = _childProcess.default.spawn(editor, args, {
            stdio: 'inherit'
        });
    }
    if (p) {
