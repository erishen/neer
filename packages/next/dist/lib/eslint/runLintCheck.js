...
var _semver = _interopRequireDefault(require("neer/dist/compiled/semver"));
var CommentJson = _interopRequireWildcard(require("neer/dist/compiled/comment-json"));
...
const requiredPackages = [
    ...
    {
        file: "eslint-config-neer",
        pkg: "eslint-config-neer",
        exportsRestrict: false
    }, 
];
async function cliPrompt() {
    ...
    try {
        const cliSelect = (await Promise.resolve(require("neer/dist/compiled/cli-select"))).default;
...
async function lint(baseDir, lintDirs, eslintrcFile, pkgJsonPath, hasAppDir, { lintDuringBuild =false , eslintOptions =null , reportErrorsOnly =false , maxWarnings =-1 , formatter =null , outputFile =null  }) {
    try {
        ...
        for (const configFile of [
            ...
            if ((ref2 = completeConfig.plugins) == null ? void 0 : ref2.includes("neer")) {
                nextEslintPluginIsEnabled = true;
                for (const [name, [severity]] of Object.entries(completeConfig.rules)){
                    if (!name.startsWith("neer/")) {
                        continue;
                    }
        ...
        const pagesDirRules = pagesDir ? [
            "neer/no-html-link-for-pages"
        ] : [];
        if (nextEslintPluginIsEnabled) {
            let updatedPagesDir = false;
            for (const rule of pagesDirRules){
                var ref3, ref4;
                if (!((ref3 = options.baseConfig.rules) == null ? void 0 : ref3[rule]) && !((ref4 = options.baseConfig.rules) == null ? void 0 : ref4[rule.replace("neer", "@next/babel-plugin-next")])) {
        ...
        return {
            ...
            eventInfo: {
                ...
                lintFix: !!options.fix,
                nextEslintPluginVersion: nextEslintPluginIsEnabled && deps.resolved.has("eslint-config-neer") ? require(_path.default.join(_path.default.dirname(deps.resolved.get("eslint-config-neer")), "package.json")).version : null,
