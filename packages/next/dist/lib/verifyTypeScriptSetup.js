...
async function verifyTypeScriptSetup({ dir , distDir , cacheDir , intentDirs , tsconfigPath , typeCheckPreflight , disableStaticImages , isAppDirEnabled  }) {
    ...
    try {
        ...
        await (0, _writeConfigurationDefaults).writeConfigurationDefaults(ts, resolvedTsConfigPath, intent.firstTimeSetup, isAppDirEnabled, distDir);
        // Write out the necessary `next-env.d.ts` file to correctly register
        // Next.js' types:
        await (0, _writeAppTypeDeclarations).writeAppTypeDeclarations(dir, !disableStaticImages);
        if (isAppDirEnabled && !_ciInfo.isCI) {
            await (0, _writeVscodeConfigurations).writeVscodeConfigurations(dir, tsPath);
        }
        let result;
        if (typeCheckPreflight) {
            const { runTypeCheck  } = require("./typescript/runTypeCheck");
            // Verify the project passes type-checking before we go to webpack phase:
            result = await runTypeCheck(ts, dir, distDir, resolvedTsConfigPath, cacheDir, isAppDirEnabled);
        }
        return {
            result,
            version: ts.version
