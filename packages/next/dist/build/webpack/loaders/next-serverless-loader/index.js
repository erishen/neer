...
const nextServerlessLoader = function() {
    ...
    const envLoading = `
      const { processEnv } = require('neer-env')
      processEnv(${Buffer.from(loadedEnvFiles, "base64").toString()})
    `;
    const runtimeConfigImports = runtimeConfig ? `
        const { setConfig } = require('neer/config')
      ` : "";
    ...
    if ((0, _isApiRoute).isAPIRoute(page)) {
        ...
        import { getApiHandler } from 'neer/dist/build/webpack/loaders/next-serverless-loader/api-handler'
      ...
      ${// this needs to be called first so its available for any other imports
        runtimeConfigSetter}
      import { getPageHandler } from 'neer/dist/build/webpack/loaders/next-serverless-loader/page-handler'
