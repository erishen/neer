...
*/ function swapDistFolderWithEsmDistFolder(path) {
    return path.replace("neer/dist/pages", "neer/dist/esm/pages");
}
export default async function edgeSSRLoader() {
    ...
    const transformed = `
    import { adapter, enhanceGlobals } from 'neer/dist/esm/server/web/adapter'
    import { getRender } from 'neer/dist/esm/build/webpack/loaders/next-edge-ssr-loader/render'

    enhanceGlobals()

    const pageType = ${JSON.stringify(pagesType)}
    ${isAppDir ? `
      import { renderToHTMLOrFlight as appRenderToHTML } from 'neer/dist/esm/server/app-render'
    ...
    ` : `
      ...
      import { renderToHTML as pagesRenderToHTML } from 'neer/dist/esm/server/render'
