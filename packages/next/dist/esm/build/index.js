import { loadEnvConfig } from "neer-env";
import chalk from "neer/dist/compiled/chalk";
import crypto from "crypto";
import { isMatch, makeRe } from "neer/dist/compiled/micromatch";
import { promises, writeFileSync } from "fs";
import { Worker as JestWorker } from "neer/dist/compiled/jest-worker";
import { Worker } from "../lib/worker";
import devalue from "neer/dist/compiled/devalue";
import { escapeStringRegexp } from "../shared/lib/escape-regexp";
import findUp from "neer/dist/compiled/find-up";
import { nanoid } from "neer/dist/compiled/nanoid/index.cjs";
import { pathToRegexp } from "neer/dist/compiled/path-to-regexp";
...
export default async function build(dir, conf = null, reactProductionProfiling = false, debugOutput = false, runLint = true, noMangling = false) {
    try {
        ...
        const buildResult = await nextBuildSpan.traceAsyncFn(async ()=>{
            ...
            if (!hasSsrAmpPages1) {
                requiredServerFiles.ignore.push(path.relative(dir, path.join(path.dirname(require.resolve("neer/dist/compiled/@ampproject/toolbox-optimizer")), "**/*")));
            }
            if (config.outputFileTracing) {
                let nodeFileTrace;
