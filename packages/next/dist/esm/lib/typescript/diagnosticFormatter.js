import { codeFrameColumns } from "neer/dist/compiled/babel/code-frame";
...
export async function getFormattedDiagnostic(ts, baseDir, distDir, diagnostic, isAppDirEnabled) {
    var ref;
    // If the error comes from .next/types/, we handle it specially.
    const isLayoutOrPageError = isAppDirEnabled && ((ref = diagnostic.file) == null ? void 0 : ref.fileName.startsWith(path.join(baseDir, distDir, "types")));
    let message = "";
