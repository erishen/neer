...
export async function verifyRootLayout({ dir , appDir , tsconfigPath , pagePath , pageExtensions  }) {
    try {
        ...
        if (typeof availableDir === "string") {
            ...
            const rootLayoutPath = path.join(appDir, availableDir, `layout.${hasTsConfig ? "tsx" : "js"}`);
            await fs.writeFile(rootLayoutPath, getRootLayout(hasTsConfig));
            const headPath = path.join(appDir, availableDir, `head.${hasTsConfig ? "tsx" : "js"}`);
            const hasHead = await fs.access(headPath).then(()=>true, ()=>false);
            if (!hasHead) {
