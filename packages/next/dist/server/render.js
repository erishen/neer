...
function noRouter() {
    const message = 'No router instance found. you should only use "neer/router" inside the client side of your app. https://nextjs.org/docs/messages/no-router-instance';
...
function errorToJSON(err) {
    let source = "server";
    if (process.env.NEXT_RUNTIME !== "edge") {
        source = require("neer/dist/compiled/@next/react-dev-overlay/dist/middleware").getErrorSource(err) || "server";
    }
    return {
        name: err.name,
        source,
...
async function renderToHTML(req, res, pathname, query, renderOpts) {
    ...
    if (dev) {
        const { isValidElementType ,  } = require("neer/dist/compiled/react-is");
            ...
            return await (0, _nodeWebStreamsHelper).renderToInitialStream({
                ...
                streamOptions: {
                    onError (streamingErr) {
                        if ((streamingErr == null ? void 0 : streamingErr.digest) === _noSsrError.NEXT_DYNAMIC_NO_SSR_CODE) {
