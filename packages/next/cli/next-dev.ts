#!/usr/bin/env node
import arg from 'neer/dist/compiled/arg/index.js'
...
const nextDev: cliCommand = async (argv) => {
  ...
  async function validateNextConfig(isCustomTurbopack: boolean) {
    ...
    const chalk =
      require('neer/dist/compiled/chalk') as typeof import('neer/dist/compiled/chalk')
...
If you cannot make the changes above, but still want to try out\nNext.js v13 with Turbopack, create the Next.js v13 playground app\nby running the following commands:
  ...
  if (args['--turbo']) {
    ...
    const findUp =
      require('neer/dist/compiled/find-up') as typeof import('neer/dist/compiled/find-up')
      ...
      startServer(devServerOptions)
        ...
        .catch((err) => {
          if (err.code === 'EADDRINUSE') {
            ...
            const pkgAppPath =
              require('neer/dist/compiled/find-up').sync(
                'package.json',
                {
                  cwd: dir,
                }
