...
async function cliPrompt() {
  ...
  try {
    const cliSelect = (
      await Promise.resolve(require('neer/dist/compiled/cli-select'))
    ).default
...
> {
  try {
    ...
    for (const configFile of [eslintrcFile, pkgJsonPath]) {
      ...
      if (completeConfig.plugins?.includes('neer')) {
      ...
      for (const rule of pagesDirRules) {
        if (
          ...
          !options.baseConfig!.rules?.[
            rule.replace('neer', '@next/babel-plugin-next')
          ]
        ) {
    ...
    return {
      ...
      eventInfo: {
        ...
        nextEslintPluginVersion:
          nextEslintPluginIsEnabled &&
          deps.resolved.has('eslint-config-neer')
            ? require(path.join(
                path.dirname(deps.resolved.get('eslint-config-neer')!),
                'package.json'
              )).version
            : null,
